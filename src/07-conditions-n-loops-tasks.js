...
function reverseInteger(num) {
  return Math.floor(num.toString().split('').reverse().join(''));
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (arr.length === 0) {
      arr.push(str[i]);
    } else if ((arr[arr.length - 1] === '(' && str[i] === ')') || (arr[arr.length - 1] === '{' && str[i] === '}') || (arr[arr.length - 1] === '[' && str[i] === ']') || (arr[arr.length - 1] === '<' && str[i] === '>')) {
      arr.pop();
    } else {
      arr.push(str[i]);
    }
...
function getCommonDirectoryPath(pathes) {
  if (pathes.length === 3) {
    if (pathes[0][0] === '/' && pathes[1][0] === '/' && pathes[2][0] === '/') {
      return '/';
    }
    return '';
  }
